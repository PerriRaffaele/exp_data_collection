...
from sanic.response import json, jsonify
import uuid
import csv
import os
app = Sanic()
...
async def upload_file(request):
    try:
        ...
        if not file or not user_id:
            return json({"error": "Invalid request. Missing file or user ID."})
...
async def submit_and_export(request):
    global ex_number
    ...
    if request.method == 'POST':
        ...
        return json({"message": "Answer data submitted successfully"})
